...
const handleParseLink = async () => {
  ...
  try {
    ...
    // 下载并设置头像（优先站点主图，其次 favicon）
    const avatarUrl = result.avatar || result.favicon;
    if (avatarUrl && avatarUploaderRef.value) {
      const previewInfo = await downloadPreviewImage(avatarUrl, 'avatar.png');
      if (previewInfo) {
