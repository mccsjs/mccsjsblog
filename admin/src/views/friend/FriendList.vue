    ...
    >
      ...
      <template #toolbar-before>
        <template v-if="!showFilter">
          ...
          <el-select
            v-model="quickFilters.is_featured"
            placeholder="推荐状态"
            clearable
            class="quick-filter-800"
            style="width: 110px"
      ...
      <el-table-column label="友链名称" min-width="130">
        <template #default="{ row }">
          ...
          <el-tag v-else-if="row.is_pending" type="info" size="small" style="margin-left: 8px"
            >待审核</el-tag
          >
          <el-tag v-if="row.is_featured" type="success" size="small" style="margin-left: 4px"
            >已推荐</el-tag
          >
        </template>
      </el-table-column>
      ...
      <el-table-column prop="sort" label="排序" width="100" align="center" class-name="hide-mobile" />

      <el-table-column label="延迟" width="100" align="center" class-name="hide-mobile">
        <template #default="{ row }">
      ...
      <el-table-column label="最新文章" width="180" align="center" class-name="hide-mobile">
        <template #default="{ row }">
      ...
      <el-table-column label="操作" :width="isMobile ? 100 : 290" align="center" :fixed="isMobile ? false : 'right'">
        <template #default="{ row }">
          <template v-if="isMobile">
            <el-dropdown trigger="click">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click="handleToggleFeatured(row)">
                    {{ row.is_featured ? '取消' : '推荐' }}
                  </el-dropdown-item>
                  <el-dropdown-item @click="handleEdit(row)">编辑</el-dropdown-item>
                  <el-dropdown-item @click="handleCheck(row)">测速</el-dropdown-item>
          ...
          <template v-else>
            ...
            <el-button type="primary" link size="small" @click="handleCheck(row)">测速</el-button>
...
import { Link, Search, Folder, ArrowDown } from '@element-plus/icons-vue';
...
import { getFriends, deleteFriend, getFriendTypes, updateFriend, checkFriend } from '@/api/friend';
...
const activeFilterCount = computed(() => {
  ...
  if (queryParams.value.is_featured !== undefined) count++;
...
const syncQuickFiltersFromQueryParams = () => {
  ...
  quickFilters.is_featured = queryParams.value.is_featured;
};
...
const handleQuickFilterChange = () => {
  ...
  queryParams.value.is_featured = quickFilters.is_featured;
  // 重置到第一页并搜索
  queryParams.value.page = 1;
  fetchFriends();
};
...
const handleToggleFeatured = async (friend: Friend) => {
  try {
    await updateFriend(friend.id, { is_featured: !friend.is_featured });
    ElMessage.success(friend.is_featured ? '已取消推荐' : '已设为推荐');
    fetchFriends();
  } catch (error) {
...
const handleCheck = async (friend: Friend) => {
  try {
    const result = await checkFriend(friend.id);
    ElMessage.success(
      result.accessible === 0
        ? `测速完成 — 延迟 ${result.latency}ms`
        : `测速完成 — 不可访问 (${result.latency}ms)`
    );
    fetchFriends();
  } catch {
...
const getLatencyClass = (latency: number): string => {
  ...
  if (latency <= 1000) return 'latency-normal';
  if (latency <= 3000) return 'latency-slow';
