...
const fetchHitokoto = async () => {
  try {
    const response = await fetch('https://api.pearapi.ai/api/hitokoto/?limit=1');
    const text = await response.text();
    // 尝试解析 JSON 数组，取第一条；否则直接使用文本
    try {
      const parsed = JSON.parse(text);
      hitokoto.value = Array.isArray(parsed) ? (parsed[0] || '获取一言失败') : (parsed.hitokoto || parsed.text || text);
    } catch {
