...
const loadBlogConfigs = async () => {
  try {
    ...
    Object.assign(blogForm.value, {
      ...
      background_image_mobile: configs.background_image_mobile || '',
...
const handleSave = async () => {
  ...
  try {
    ...
    if (blogUploaders) {
      ...
      if (blogUploaders.mobileBackgroundUploaderRef?.getPendingCount()) {
        uploadPromises.push(
          blogUploaders.mobileBackgroundUploaderRef.uploadPendingFile().then(url => {
            if (url) blogForm.value.background_image_mobile = url;
          })
        );
      }
    ...
    const blogPayload: Record<string, string> = {
      ...
      'blog.background_image_mobile': blogForm.value.background_image_mobile,
