...
function renderVideo(params: string[], sourceAttrs = ''): string {
  ...
  if (platformOrUrl === 'bilibili' && videoId) {
    return `<div class="custom-video"${sourceAttrs}><iframe src="//player.bilibili.com/player.html?bvid=${videoId}&autoplay=0&high_quality=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" ></iframe></div>`;
  }

  // YouTube视频
  if (platformOrUrl === 'youtube' && videoId) {
    return `<div class="custom-video"${sourceAttrs}><iframe src="https://www.youtube.com/embed/${videoId}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen ></iframe></div>`;
  }
