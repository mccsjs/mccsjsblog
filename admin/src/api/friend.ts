 ...
 * 测速单个友链
 * @param id 友链ID
 * @returns Promise<{ accessible: number; latency: number }>
 */
export function checkFriend(id: number): Promise<{ accessible: number; latency: number }> {
  return request.post(`/admin/friends/${id}/check`);
}
