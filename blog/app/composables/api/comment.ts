...
/** 切换评论置顶状态（超级管理员） */
export const toggleCommentPinned = async (id: number) => {
  return put(`/comments/${id}/toggle-pinned`);
};

/** 切换评论显示状态（超级管理员） */
export const toggleCommentStatus = async (id: number) => {
  return put(`/comments/${id}/toggle-status`);
};
