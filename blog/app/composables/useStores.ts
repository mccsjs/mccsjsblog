...
export function useArticles() {
  ...
  const pageSize = useState<number>('articles-pageSize', () => 8);
...
export function useFriends() {
  const friendGroups = useState<FriendGroup[]>('friends', () => []);

  const fetchFriends = async (forceRefresh = false) => {
    if (!forceRefresh && friendGroups.value.length) return;
  ...
  return { friendGroups, fetchFriends };
}
