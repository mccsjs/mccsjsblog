...
const avatarUrl = computed(() => basicConfig.value.author_avatar || '/avatar.jpg');
...
const MOMENT_FANCYBOX_ATTR = 'data-fancybox-moment';
...
const initFancybox = async () => {
  ...
  // 给图片添加 data-fancybox 属性用于分组
  images.forEach(img => {
    if (!img.hasAttribute('data-fancybox')) {
      img.setAttribute('data-fancybox', MOMENT_FANCYBOX_ATTR);
    }
  });
  ...
  Fancybox.bind(`[data-fancybox="${MOMENT_FANCYBOX_ATTR}"]`, {
    groupAll: true,
    Thumbs: false,
    Toolbar: {
      display: {
...
#moment-page {
  ...
  .moment-header {
    ...
    align-items: center;
    padding: 0.5rem 1rem;
    border-bottom: 1px solid var(--mccsjs-moment-divider);
    ...
    .moment-meta {
      ...
      .moment-author {
        font-weight: 600;
        color: var(--mccsjs-moment-title);
      }

      .moment-time {
        font-size: 0.875rem;
        color: var(--mccsjs-moment-date);
      }
    }
  }
  ...
  .moment-content {
    ...
    .moment-audio {
      ...
      border-radius: 6px;
      overflow: hidden;
      background: var(--mccsjs-moment-card-bg);
      transition: transform 0.3s ease;
