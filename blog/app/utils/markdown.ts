...
function extractTagAndParams(line: string): { tag: string; params: string[] } {
  ...
  // 简单按空格分割参数
  const params = paramsString ? paramsString.split(/\s+/).filter(p => p && p !== ":::").map(p => p.replace(/:::/g, "")) : [];
...
md.renderer.rules.fence = (tokens, idx) => {
  ...
  const collapsedClass = lineCount > 10 ? ' collapsed' : '';

  // 返回完整结构
  const foldBtns = lineCount > 10
  ? '<button class="code-expand-btn" onclick="this.closest(\'.code-block-container\').classList.remove(\'collapsed\')"><i class="ri-arrow-down-s-line"></i>展开</button><button class="code-collapse-btn" onclick="this.closest(\'.code-block-container\').classList.add(\'collapsed\')"><i class="ri-arrow-up-s-line"></i>折叠</button>'
  : '';

return `<div class="code-block-container${collapsedClass}" data-code-content="${escapedCode}"><div class="code-toolbar"><span class="code-lang">${displayLang}</span><button class="code-copy-btn" onclick="copyCodeBlock(this)" title="复制代码"><i class="ri-file-copy-fill"></i></button></div><pre><code>${numberedLines}</code></pre>${foldBtns}</div>`;
};
...
export function renderSimpleMarkdown(markdown: string): string {
  ...
  // 重写图片渲染规则，添加懒加载属性
  const defaultSimpleImageRenderer = simpleMd.renderer.rules.image || function (tokens, idx, options, env, self) {
    return self.renderToken(tokens, idx, options);
  };
  simpleMd.renderer.rules.image = function (tokens, idx, options, env, self) {
    const token = tokens[idx];
    if (!token) return '';
    token.attrSet('loading', 'lazy');
    token.attrSet('decoding', 'async');
    return defaultSimpleImageRenderer(tokens, idx, options, env, self);
  };
  ...
  return DOMPurify.sanitize(simpleHtml, {
    ...
    ALLOWED_ATTR: ['href', 'title', 'src', 'alt', 'width', 'height', 'class', 'loading', 'decoding'],
    ALLOWED_URI_REGEXP:
    ...
    fetch(embedUrl)
      ...
      .then(data => {
        if (data && data.length > 0) {
          const info = data[0];
          if (musicInfoEl) {
            musicInfoEl.textContent = `${info.title || info.name || '未知歌曲'} - ${info.artist || info.author || '未知艺术家'}`;
          }
