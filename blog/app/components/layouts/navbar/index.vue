...
const isHomePage = computed(() => route.meta.typeHeader === 'home');
...
<template>
  <nav id="navbar" :class="{ fixed: isFixed, 'no-header': !showHeader, 'is-home': isHomePage }">
    <div class="nav-left">
...
<style lang="scss">
@mixin text-color {
  color: var(--mccsjs-nav-font);

  &:hover {
    color: var(--mccsjs-nav-font-hover);
  }
}
...
#navbar {
  ...
  &.no-header {
    background-color: var(--mccsjs-nav-bg);
  ...
  .brighten {
    ...
    &:after {
      ...
      width: 90%;
      height: 1px;
      background-color: var(--mccsjs-nav-focus);
