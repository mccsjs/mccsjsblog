...
const metingApi = computed(() => blogConfig.value.meting_api || 'https://api.i-meto.com/meting/api');
...
const pauseOtherAudio = () => {
  ...
  document.querySelectorAll('audio').forEach((el) => {
    if (el !== audioRef.value && !el.paused) el.pause();
  });
};
...
.player-playlist {
  max-height: 120px;
  overflow-y: auto;
  border-top: 1px solid var(--mccsjs-moment-divider, rgba(0, 0, 0, 0.06));
