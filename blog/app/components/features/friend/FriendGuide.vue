...
const toFrontendUrl = (url?: string) => {
  if (!url) return '';
  // 如果是相对路径或已经是前端地址，直接返回
  if (url.startsWith('/') || !url.includes('localhost:8080')) return url;
  // 将后端地址转换为前端地址：提取 /uploads/... 部分
  try {
    const urlObj = new URL(url);
    return `${requestURL.origin}${urlObj.pathname}`;
  } catch {
    return url;
  }
};
...
const siteConfig = computed(() => ({
  ...
  avatar: toFrontendUrl(basicConfig.value.author_avatar) || blogConfig.value.favicon,
  description: blogConfig.value.slogan,
  screenshot: toFrontendUrl(blogConfig.value.screenshot),
}));
...
<template>
  ...
  <ol>
    <li>
      已添加<NuxtLink to="/" class="highlight">本博客</NuxtLink>的友情链接
    </li>
    <li>网站内容积极向上正能量并符合中华人民共和国法律</li>
    <li>网站可以在 1 分钟内加载完成首屏</li>
  </ol>
  ...
  <div class="tabs">
    <div class="tab-buttons">
      <button :class="{ active: activeTab === 'yaml' }" @click="activeTab = 'yaml'">YAML</button>      <button :class="{ active: activeTab === 'table' }" @click="activeTab = 'table'">通用</button>
    </div>
...
网站截图（可选）：</code></pre>
      ...
      <div v-show="activeApplyTab === 'form'" class="apply-format">
        <div class="form-apply-content">
          <div>
            <p class="title">推荐使用在线申请</p>
            <p>
              使用在线申请更加便捷！只需填写您网站的基本信息，系统会收到您的申请并通知管理员，减少信息填写错误，管理员会尽快处理您的申请。
            </p>
  ...
  >
    <form @submit.prevent="submitApplyForm">
      ...
      <div class="form-group">
        <label for="site-screenshot">网站截图（未提供将自动截图）</label>
        <input
          id="site-screenshot"
...
.tabs {
  ...
  .tab-buttons {
    ...
    margin-bottom: 12px;
    padding-bottom: 12px;
    border-bottom: 1px solid var(--mccsjs-heavy-bg);
  ...
  .tab-content {
    ...
    .table-content {
      ...
      table {
        ...
        tr {
          &:not(:last-child) td {
            border-bottom: 1px solid var(--mccsjs-card-bg);
          }
        }
