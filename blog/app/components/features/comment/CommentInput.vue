...
const mainBtn = computed(() => {
  ...
  return { text: '发送', icon: 'ri-send-plane-fill' };
});

const secondaryBtn = computed(() => ({ text: '登录', icon: 'ri-login-box-line' }));
...
const handleMainAction = () => handleSubmitComment();
...
.guest-policy-trigger {
  ...
  height: 24px;
  padding: 0;
  border: 1px solid var(--mccsjs-border-color);
  border-radius: 999px;
...
.submit-btn {
  ...
  &:hover:not(:disabled) {
    background: var(--mccsjs-btn-hover);
  }
...
@media screen and (max-width: 768px) {
  :deep(.reply-mode .emoji-picker-portal .emoji-tabs) {
    flex-wrap: nowrap;
    overflow-x: auto;
    -webkit-overflow-scrolling: touch;
    scrollbar-width: none;
  }
  :deep(.reply-mode .emoji-picker-portal .emoji-tabs::-webkit-scrollbar) { display: none; }
  :deep(.reply-mode .emoji-picker-portal .emoji-tab) {
    flex-shrink: 0;
    white-space: nowrap;
  }
