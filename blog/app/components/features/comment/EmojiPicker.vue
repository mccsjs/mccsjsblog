...
// 表情 hover 预览
const preview = reactive({ visible: false, src: '', x: 0, y: 0 });

const showPreview = (e: MouseEvent, src: string) => {
  const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
  const size = 100;
  preview.src = src;
  preview.x = rect.left + rect.width / 2 - size / 2;
  preview.y = rect.top - size - 8;
  // 边界修正
  if (preview.x < 4) preview.x = 4;
  if (preview.x + size > window.innerWidth - 4) preview.x = window.innerWidth - size - 4;
  if (preview.y < 4) preview.y = rect.bottom + 8;
...
<template>
  <div class="emoji-picker">
      ...
      <div class="emoji-content" @wheel.stop>
        ...
        >
          <button
            ...
            @mouseenter="group.type === 'image' && showPreview($event, item.val)"
            @mouseleave="group.type === 'image' && hidePreview()"
          >
    ...
    <!-- hover 预览浮窗 -->
    <Teleport to="body">
      <div v-if="preview.visible" class="emoji-hover-preview" :style="{ left: preview.x + 'px', top: preview.y + 'px' }">
        <img :src="preview.src" />
      </div>
    </Teleport>
  </div>
...
.emoji-content {
  ...
  &::-webkit-scrollbar-thumb {
    background: var(--mccsjs-border-color);
    border-radius: 3px;
  }
}
