...
import (
	...
	"blog/pkg/email"
	"blog/pkg/feishu"
	mcpserver "blog/pkg/mcp"
	"blog/pkg/notification"
	"blog/pkg/scheduler"
	"blog/pkg/upload"
...
// InitRouter 初始化路由
func InitRouter(db *database.Database, conf *config.Config, cacheClient *cache.Client) *gin.Engine {
	// 设置为 release 模式
	...
	comicRepo := repository.NewComicRepository(db.DB)
	...
	// 根路径欢迎页面
	r.GET("/", func(c *gin.Context) { c.String(200, "Mccsjs-Server 运行成功") })
	...
	articleService := service.NewArticleService(articleRepo, tagRepo, categoryRepo, commentRepo, fileService, db.DB, cacheClient)
	...
	friendService := service.NewFriendService(friendRepo, fileService, notificationService, cacheClient)
	comicService := service.NewComicService(comicRepo)
	...
	friendController := v1.NewFriendController(friendService, rssFeedService)
	comicController := v1.NewComicController(comicService)
	...
	frontendAPI := r.Group("/api/v1")
	{
		...
		{
			// 公开接口
			friendGroup.GET("/feed", friendController.Feed)      // 朋友圈文章流
		...
		// ==================== 番剧相关 ====================
		comicGroup := frontendAPI.Group("/comics")
		{
			comicGroup.GET("", comicController.ListForWeb)
		}
		...
		commentGroup := frontendAPI.Group("/comments")
		{
			...
			// 超级管理员接口
			superAdminComment := commentGroup.Group("")
			superAdminComment.Use(middleware.Auth(userService), middleware.IsSuperAdmin())
			{
				superAdminComment.PUT("/:id/toggle-pinned", commentController.TogglePinned) // 切换置顶
				superAdminComment.PUT("/:id/toggle-status", commentController.ToggleStatus) // 切换状态
			}
		}
	...
	adminAPI.Use(middleware.Auth(userService), middleware.IsAdminOrAbove())
	{
		...
		friendManagement := adminAPI.Group("/friends")
		{
			...
			friendManagement.POST("/:id/check", friendController.Check)   // 测速友链
		}
		...
		momentManagement := adminAPI.Group("/moments")
		{
			...
			// ==================== 番剧管理 ====================
			comicManagement := adminAPI.Group("/comics")
			{
				comicManagement.GET("", comicController.AdminList)     // 获取番剧列表
				comicManagement.GET("/:id", comicController.Get)       // 获取番剧详情
				comicManagement.POST("", comicController.Create)       // 创建番剧
				comicManagement.PUT("/:id", comicController.Update)    // 更新番剧
				comicManagement.DELETE("/:id", comicController.Delete) // 删除番剧
		}
		...
		commentManagement := adminAPI.Group("/comments")
		{
			...
			commentManagement.PUT("/:id/toggle-pinned", commentController.TogglePinned) // 切换评论置顶
		...
		rssFeedManagement := adminAPI.Group("/rssfeed")
		{
			...
			rssFeedManagement.POST("/refresh", rssFeedController.Refresh)                                     // 手动刷新RSS订阅源
...
func initScheduler(fileService *service.FileService, userService *service.UserService, verificationService *service.VerificationService, rssFeedService *service.RssFeedService, friendService *service.FriendService) {
	...
	// 友链检测任务（每天0点和12点，批量测速所有友链）
	_ = s.AddJob(scheduler.NewJob("友链状态检测", "0 0 0,12 * * *", friendService.CheckAllFriends))
