...
func NewFriendController(friendService *service.FriendService, rssFeedService *service.RssFeedService) *FriendController {
	return &FriendController{friendService: friendService, rssFeedService: rssFeedService}
}
...
//
//	@Summary		朋友圈
//	@Description	获取所有友链的最新RSS文章，按发布时间倒序
//	@Tags			友链
//	@Produce		json
//	@Param			page		query	int	false	"页码"
//	@Param			page_size	query	int	false	"每页数量"
//	@Success		200			{object}	response.Response{data=dto.FriendFeedResponse}
//	@Router			/friends/feed [get]
func (c *FriendController) Feed(ctx *gin.Context) {
	page := 1
	pageSize := 20
	if p, err := strconv.Atoi(ctx.DefaultQuery("page", "1")); err == nil && p > 0 {
		page = p
	}
	if ps, err := strconv.Atoi(ctx.DefaultQuery("page_size", "20")); err == nil && ps > 0 && ps <= 100 {
		pageSize = ps
	}

	result, err := c.rssFeedService.GetFriendFeed(ctx.Request.Context(), page, pageSize)
	if err != nil {
...
func (c *FriendController) UpdateType(ctx *gin.Context) {
	...
	ctx.Header("X-Data-Updated", "stats")
...
//
//	@Summary		测速友链 [测速]
//	@Description	检测单个友链的可访问性和响应延迟
//	@Tags			友链管理
//	@Produce		json
//	@Security		BearerAuth
//	@Param			id	path		int	true	"友链 ID"
//	@Success		200	{object}	response.Response{data=dto.FriendCheckResponse}
//	@Failure		400	{object}	response.Response
//	@Failure		401	{object}	response.Response
//	@Failure		403	{object}	response.Response
//	@Router			/admin/friends/{id}/check [post]
func (c *FriendController) Check(ctx *gin.Context) {
	id, err := strconv.ParseUint(ctx.Param("id"), 10, 32)
	if err != nil {
		response.ValidateFailed(ctx, err.Error())
		return
	...
	accessible, latency, err := c.friendService.CheckFriend(ctx.Request.Context(), uint(id))
	if err != nil {
		response.Failed(ctx, err.Error())
		return
	}

	response.Success(ctx, dto.FriendCheckResponse{
