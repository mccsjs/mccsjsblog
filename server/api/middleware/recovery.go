...
func formatStackTrace(stack string, simplified bool) string {
	...
	for i := 0; i < len(lines); i++ {
		...
		if simplified {
			// 只保留包含 blog 的行（项目代码）
			if !strings.Contains(trimmedLine, "blog") {
				continue
			}
...
func extractKeyStack(stack string) string {
	...
	for i := 0; i < len(lines)-1; i++ {
		...
		// 找到项目代码
		if strings.Contains(line, "blog") && !strings.Contains(line, "middleware/recovery.go") {
			// 下一行通常是文件位置
			if i+1 < len(lines) {
				...
				if strings.Contains(locationLine, ".go:") {
					// 简化路径：只保留相对路径
					parts := strings.Split(locationLine, "blog/server/")
					if len(parts) > 1 {
						return parts[1]
					}
					return locationLine
