...
// GetLastUpdateTime 获取最后更新时间（取文章和友链中最近的更新时间）
func (r *StatsRepository) GetLastUpdateTime() (*time.Time, error) {
	var articleTime, friendTime *time.Time

	// 查询文章最近更新时间（同时考虑 update_time 和 updated_at）
	err := r.db.Model(&model.Article{}).
		Select("GREATEST(COALESCE(MAX(update_time), '1970-01-01'), COALESCE(MAX(updated_at), '1970-01-01'))").
		Scan(&articleTime).Error
	if err != nil {
		return nil, err
	}
	...
	err = r.db.Model(&model.Friend{}).
		...
		Scan(&friendTime).Error
	if err != nil {
		return nil, err
	}

	// 取较新的时间
	if articleTime == nil && friendTime == nil {
		return nil, nil
	}
	if articleTime == nil {
		return friendTime, nil
	}
	if friendTime == nil {
		return articleTime, nil
	}
	if friendTime.After(*articleTime) {
		return friendTime, nil
	}
	return articleTime, nil
}
