...
// UpdateCheckStatus 更新友链检测状态和延迟
func (r *FriendRepository) UpdateCheckStatus(ctx context.Context, id uint, accessible int, latency int) error {
...
// UpdateRSSUrl 更新友链的 RSS 地址（自动发现）
func (r *FriendRepository) UpdateRSSUrl(ctx context.Context, id uint, rssUrl string) error {
	return r.db.WithContext(ctx).
		Model(&model.Friend{}).
		Where("id = ?", id).
		UpdateColumn("rss_url", rssUrl).Error
}
