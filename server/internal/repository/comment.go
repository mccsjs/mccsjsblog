...
func (r *CommentRepository) TogglePinned(ctx context.Context, id uint) (int64, error) {
	// 先查询当前评论的置顶状态
	var comment model.Comment
	if err := r.db.WithContext(ctx).Select("is_pinned").First(&comment, id).Error; err != nil {
		return 0, err
	}
	...
	// 如果不是置顶状态，先取消所有评论的置顶，再置顶当前评论
	if err := tx.Model(&model.Comment{}).Where("is_pinned = ?", true).UpdateColumn("is_pinned", false).Error; err != nil {
		tx.Rollback()
		return 0, err
	}
