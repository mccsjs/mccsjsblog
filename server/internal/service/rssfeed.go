...
// GetFriendFeed 获取朋友圈文章流（公开接口）
func (s *RssFeedService) GetFriendFeed(ctx context.Context, page, pageSize int) (*dto.FriendFeedResponse, error) {
	if page <= 0 {
		page = 1
	}
	...
	articles, total, err := s.repo.List(ctx, page, pageSize, "", 0, nil, "", "")
	if err != nil {
		return nil, err
	}

	list := make([]dto.FriendFeedItem, 0, len(articles))
	for _, article := range articles {
		item := dto.FriendFeedItem{
			ID:          article.ID,
		...
		if article.Friend != nil {
			...
			item.FriendAvatar = article.Friend.Avatar
		}
		list = append(list, item)
	}

	return &dto.FriendFeedResponse{
		List:     list,
		Total:    total,
		Page:     page,
		PageSize: pageSize,
