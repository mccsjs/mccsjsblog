package service

import (
	"context"
	"encoding/json"
	"errors"
	"io"
	"net/http"
	"net/url"
	"strconv"
	"strings"
	"time"

	"github.com/PuerkitoBio/goquery"

	"blog/internal/dto"
	"blog/internal/model"
	"blog/internal/repository"
	"blog/pkg/cache"
	"blog/pkg/logger"
	"blog/pkg/utils"
)

// FriendService 友链服务
type FriendService struct {
	repo                *repository.FriendRepository
	fileService         *FileService
	notificationService *NotificationService
	cacheClient         *cache.Client
}

// NewFriendService 创建友链服务实例
func NewFriendService(repo *repository.FriendRepository, fileService *FileService, notificationService *NotificationService, cacheClient *cache.Client) *FriendService {
	return &FriendService{
		repo:                repo,
		fileService:         fileService,
		notificationService: notificationService,
		cacheClient:         cacheClient,
	}
}

// ============ 友链类型 ============

// ListTypes 获取所有友链类型（后台管理用）
func (s *FriendService) ListTypes(ctx context.Context, page, pageSize int) ([]dto.FriendTypeListResponse, int64, error) {
	types, total, err := s.repo.ListTypes(ctx, page, pageSize)
	if err != nil {
		return nil, 0, err
	}

	result := make([]dto.FriendTypeListResponse, 0, len(types))
	for _, t := range types {
		// 统计该类型下的友链数量
		count, err := s.repo.CountFriendsByTypeID(ctx, t.ID)
		if err != nil {
			count = 0
		}

		result = append(result, dto.FriendTypeListResponse{
			ID:        t.ID,
			Name:      t.Name,
			Sort:      t.Sort,
			IsVisible: t.IsVisible,
			Count:     int(count),
		})
	}
	return result, total, nil
}

// GetType 获取友链类型详情
func (s *FriendService) GetType(ctx context.Context, id uint) (*model.FriendType, error) {
	return s.repo.GetTypeByID(ctx, id)
}

// CreateType 创建友链类型
func (s *FriendService) CreateType(ctx context.Context, req *dto.CreateFriendTypeRequest) error {
	friendType := &model.FriendType{
		Name:      req.Name,
		Sort:      req.Sort,
		IsVisible: true, // 默认展示
	}

	if req.IsVisible != nil {
		friendType.IsVisible = *req.IsVisible
	}

	return s.repo.CreateType(ctx, friendType)
}

// UpdateType 更新友链类型
func (s *FriendService) UpdateType(ctx context.Context, id uint, req *dto.UpdateFriendTypeRequest) error {
	// 检查类型是否存在
	existingType, err := s.repo.GetTypeByID(ctx, id)
	if err != nil {
		return errors.New("友链类型不存在")
	}

	// 更新字段
	existingType.Name = req.Name
	existingType.Sort = req.Sort

	if req.IsVisible != nil {
		existingType.IsVisible = *req.IsVisible
	}

	return s.repo.UpdateType(ctx, existingType)
}

// DeleteType 删除友链类型
func (s *FriendService) DeleteType(ctx context.Context, id uint) error {
	// 检查类型是否存在
	_, err := s.repo.GetTypeByID(ctx, id)
	if err != nil {
		return errors.New("友链类型不存在")
	}

	// 注意：由于外键设置了 ON DELETE SET NULL，删除类型时，关联的友链的 type_id 会被设置为 NULL
	return s.repo.DeleteType(ctx, id)
}

// ============ 前台服务 ============

// ListForWeb 获取友链分组列表（包括失效友链，前端可根据 is_invalid 字段处理展示）
func (s *FriendService) ListForWeb(ctx context.Context) (*dto.GroupedFriendsResponse, error) {
	// 0. 尝试从缓存获取
	if s.cacheClient != nil {
		cacheKey := cache.BuildKey("friends", "groups")
		var cached dto.GroupedFriendsResponse
		if err := s.cacheClient.Get(ctx, cacheKey, &cached); err == nil {
			return &cached, nil
		}
	}

	// 1. 获取所有数据
	types, allFriends, err := s.repo.ListForWeb(ctx)
	if err != nil {
		return nil, err
	}

	// 2. 在内存中按类型分组（使用 0 表示未分类）
	const uncategorizedKey uint = 0
	typeMap := make(map[uint][]model.Friend)

	for _, friend := range allFriends {
		key := uncategorizedKey
		if friend.TypeID != nil {
			key = *friend.TypeID
		}
		typeMap[key] = append(typeMap[key], friend)
	}

	// 3. 组装响应数据（空分组不返回）
	groups := make([]dto.FriendGroupResponse, 0, len(types)+1)
	totalFriends := 0

	for _, t := range types {
		if friends := typeMap[t.ID]; len(friends) > 0 {
			groups = append(groups, buildGroupResponse(&t.ID, t.Name, t.Sort, friends))
			totalFriends += len(friends)
		}
	}

	// 添加未分类友链
	if friends := typeMap[uncategorizedKey]; len(friends) > 0 {
		groups = append(groups, buildGroupResponse(nil, "友情链接", 0, friends))
		totalFriends += len(friends)
	}

	result := &dto.GroupedFriendsResponse{
		Groups:       groups,
		TotalGroups:  len(groups),
		TotalFriends: totalFriends,
	}

	// 写入缓存
	if s.cacheClient != nil {
		cacheKey := cache.BuildKey("friends", "groups")
		_ = s.cacheClient.Set(ctx, cacheKey, result)
	}

	return result, nil
}

// buildGroupResponse 构建分组响应（辅助函数）
func buildGroupResponse(typeID *uint, name string, sort int, friends []model.Friend) dto.FriendGroupResponse {
	return dto.FriendGroupResponse{
		TypeID:   typeID,
		TypeName: name,
		TypeSort: sort,
		Friends:  convertToFriendInGroupResponse(friends),
	}
}

// convertToFriendInGroupResponse 转换为分组友链响应格式（辅助函数）
func convertToFriendInGroupResponse(friends []model.Friend) []dto.FriendInGroupResponse {
	result := make([]dto.FriendInGroupResponse, len(friends))
	for i, f := range friends {
		screenshot := f.Screenshot
		if screenshot == "" && f.URL != "" {
			screenshot = "https://s0.wp.com/mshots/v1/" + url.QueryEscape(f.URL) + "?w=400&h=300"
		}
		result[i] = dto.FriendInGroupResponse{
			ID:          f.ID,
			Name:        f.Name,
			URL:         f.URL,
			Description: f.Description,
			Avatar:      f.Avatar,
			Screenshot:  screenshot,
			Sort:        f.Sort,
			IsInvalid:   f.IsInvalid,
			Accessible:  f.Accessible,
			Latency:     f.Latency,
			IsFeatured:  f.IsFeatured,
		}
	}
	return result
}

// ============ 后台管理服务 ============

// List 获取友链列表
func (s *FriendService) List(ctx context.Context, req *dto.ListFriendRequest) ([]dto.FriendListResponse, int64, error) {
	friends, total, err := s.repo.List(ctx, req.Page, req.PageSize, req.Keyword, req.TypeID, req.IsInvalid, req.IsPending, req.AccessibleStatus, req.RSSStatus, req.HasScreenshot)
	if err != nil {
		return nil, 0, err
	}

	result := make([]dto.FriendListResponse, 0, len(friends))
	for _, friend := range friends {
		response := dto.FriendListResponse{
			ID:          friend.ID,
			Name:        friend.Name,
			URL:         friend.URL,
			Description: friend.Description,
			Avatar:      friend.Avatar,
			Screenshot:  friend.Screenshot,
			Sort:        friend.Sort,
			IsInvalid:   friend.IsInvalid,
			IsPending:   friend.IsPending,
			TypeID:      friend.TypeID,
			RSSUrl:      friend.RSSUrl,
			Accessible:  friend.Accessible,
			Latency:     friend.Latency,
			IsFeatured:  friend.IsFeatured,
		}

		// 如果有类型，返回类型名称（后台展示用）
		if friend.Type != nil {
			response.TypeName = friend.Type.Name
		}

		// 如果有RSS最后更新时间，格式化返回
		response.RSSLatime = utils.ToJSONTime(friend.RSSLatime)

		result = append(result, response)
	}

	return result, total, nil
}

// Get 获取友链详情
func (s *FriendService) Get(ctx context.Context, id uint) (*model.Friend, error) {
	return s.repo.Get(ctx, id)
}

// Create 创建友链
func (s *FriendService) Create(ctx context.Context, req *dto.CreateFriendRequest) (*model.Friend, error) {
	if req.Sort == 0 {
		req.Sort = 5
	}

	// 如果指定了类型，验证类型是否存在
	if req.TypeID != nil {
		_, err := s.repo.GetTypeByID(ctx, *req.TypeID)
		if err != nil {
			return nil, errors.New("指定的友链类型不存在")
		}
	}

	friend := &model.Friend{
		Name:        req.Name,
		URL:         req.URL,
		Description: req.Description,
		Avatar:      req.Avatar,
		Screenshot:  req.Screenshot,
		Sort:        req.Sort,
		IsInvalid:   false, // 新创建的友链默认不失效
		TypeID:      req.TypeID,
		RSSUrl:      req.RSSUrl,
		IsFeatured:  req.IsFeatured,
	}

	if err := s.repo.Create(ctx, friend); err != nil {
		return nil, err
	}

	// 标记文件为使用中
	if s.fileService != nil {
		if req.Avatar != "" {
			_ = s.fileService.MarkAsUsed(req.Avatar)
		}
		if req.Screenshot != "" {
			_ = s.fileService.MarkAsUsed(req.Screenshot)
		}
	}

	return friend, nil
}

// Update 更新友链
func (s *FriendService) Update(ctx context.Context, id uint, req *dto.UpdateFriendRequest) error {
	// 检查友链是否存在
	existingFriend, err := s.repo.Get(ctx, id)
	if err != nil {
		return errors.New("友链不存在")
	}

	// 如果指定了类型，验证类型是否存在
	if req.TypeID != nil {
		_, err := s.repo.GetTypeByID(ctx, *req.TypeID)
		if err != nil {
			return errors.New("指定的友链类型不存在")
		}
	}

	// 保存旧文件
	oldAvatar := existingFriend.Avatar
	oldScreenshot := existingFriend.Screenshot

	// 更新字段（仅当请求中提供且非空时更新）
	if req.Name != "" {
		existingFriend.Name = req.Name
	}
	if req.URL != "" {
		existingFriend.URL = req.URL
	}
	if req.Description != "" {
		existingFriend.Description = req.Description
	}
	if req.Avatar != "" {
		existingFriend.Avatar = req.Avatar
	}
	if req.Screenshot != "" {
		existingFriend.Screenshot = req.Screenshot
	}
	if req.Sort != nil {
		existingFriend.Sort = *req.Sort
	}
	if req.TypeID != nil {
		existingFriend.TypeID = req.TypeID
	}
	if req.RSSUrl != "" {
		existingFriend.RSSUrl = req.RSSUrl
	}
	existingFriend.UpdatedAt = time.Now()

	// 如果请求中指定了失效状态
	if req.IsInvalid != nil {
		existingFriend.IsInvalid = *req.IsInvalid
		// 如果标记为失效，同时清除异常状态
		if *req.IsInvalid {
			existingFriend.Accessible = 0
		}
	}

	// 如果请求中指定了待审核状态
	if req.IsPending != nil {
		existingFriend.IsPending = *req.IsPending
	}

	// 如果请求中指定了可访问性状态
	if req.Accessible != nil && !existingFriend.IsInvalid {
		existingFriend.Accessible = *req.Accessible
	}

	// 如果请求中指定了推荐状态
	if req.IsFeatured != nil {
		existingFriend.IsFeatured = *req.IsFeatured
	}

	if err := s.repo.Update(ctx, existingFriend); err != nil {
		return err
	}

	// 处理文件变化
	if s.fileService != nil {
		if oldAvatar != existingFriend.Avatar {
			if oldAvatar != "" {
				_ = s.fileService.MarkAsUnused(oldAvatar)
			}
			if existingFriend.Avatar != "" {
				_ = s.fileService.MarkAsUsed(existingFriend.Avatar)
			}
		}
		if oldScreenshot != existingFriend.Screenshot {
			if oldScreenshot != "" {
				_ = s.fileService.MarkAsUnused(oldScreenshot)
			}
			if existingFriend.Screenshot != "" {
				_ = s.fileService.MarkAsUsed(existingFriend.Screenshot)
			}
		}
	}

	return nil
}

// Delete 删除友链
func (s *FriendService) Delete(ctx context.Context, id uint) error {
	// 检查友链是否存在
	friend, err := s.repo.Get(ctx, id)
	if err != nil {
		return errors.New("友链不存在")
	}

	if err := s.repo.Delete(ctx, id); err != nil {
		return err
	}

	// 标记文件为未使用
	if s.fileService != nil {
		if friend.Avatar != "" {
			_ = s.fileService.MarkAsUnused(friend.Avatar)
		}
		if friend.Screenshot != "" {
			_ = s.fileService.MarkAsUnused(friend.Screenshot)
		}
	}

	return nil
}

// ============ 前台友链申请 ============

// ApplyFriend 申请友链（前台用户提交申请）
func (s *FriendService) ApplyFriend(ctx context.Context, req *dto.ApplyFriendRequest, userID uint) error {
	// 创建待审核的友链记录
	friend := &model.Friend{
		Name:        req.Name,
		URL:         req.URL,
		Description: req.Description,
		Avatar:      req.Avatar,
		Screenshot:  req.Screenshot,
		IsPending:   true, // 标记为待审核
	}

	if err := s.repo.Create(ctx, friend); err != nil {
		return err
	}

	if s.fileService != nil {
		if req.Avatar != "" {
			_ = s.fileService.MarkAsUsed(req.Avatar)
		}
		if req.Screenshot != "" {
			_ = s.fileService.MarkAsUsed(req.Screenshot)
		}
	}

	// 发送友链申请通知给管理员
	if s.notificationService != nil {
		_ = s.notificationService.NotifyFriendApply(ctx, friend.ID, req.Name, req.URL, req.Description, req.Avatar, req.Screenshot, &userID)
	}

	return nil
}

// ApproveFriend 通过友链申请（将待审核状态改为已通过）
func (s *FriendService) ApproveFriend(ctx context.Context, id uint) error {
	// 检查友链是否存在
	friend, err := s.repo.Get(ctx, id)
	if err != nil {
		return errors.New("友链不存在")
	}

	// 将待审核状态改为 false
	friend.IsPending = false
	friend.UpdatedAt = time.Now()

	if err := s.repo.Update(ctx, friend); err != nil {
		return err
	}

	// 异步检测新通过的友链状态，不阻塞 API 响应
	go s.checkOneFriend(friend)

	return nil
}

// checkOneFriend 检测单个友链（异步调用，带退避重试）
func (s *FriendService) checkOneFriend(friend *model.Friend) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()

	s.doCheckOne(ctx, friend)
}

// checkOneFriendWithTimeout 检测单个友链，可指定超时
func (s *FriendService) checkOneFriendWithTimeout(friend *model.Friend, timeout time.Duration) {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()

	s.doCheckOne(ctx, friend)
}

// CheckFriend 对外暴露的单友链测速（由管理端按钮触发）
// 返回可访问性状态和延迟
func (s *FriendService) CheckFriend(ctx context.Context, id uint) (accessible int, latency int, err error) {
	friend, err := s.repo.Get(ctx, id)
	if err != nil {
		return 0, 0, errors.New("友链不存在")
	}

	newStatus, latency := s.dualCheck(ctx, friend.URL)
	_ = s.repo.UpdateCheckStatus(ctx, friend.ID, newStatus, latency)

	// 可达时尝试自动发现 RSS
	if newStatus == 0 {
		s.discoverAndSaveRSS(ctx, friend)
	}

	return newStatus, latency, nil
}

// doCheckOne 执行单个友链检测（直连 + API 双路检测，取最优）
func (s *FriendService) doCheckOne(ctx context.Context, friend *model.Friend) {
	newStatus, latency := s.dualCheck(ctx, friend.URL)
	_ = s.repo.UpdateCheckStatus(ctx, friend.ID, newStatus, latency)

	// 可达时尝试自动发现 RSS
	if newStatus == 0 {
		s.discoverAndSaveRSS(ctx, friend)
	}
}

// dualCheck 双路检测：API 和直连同时尝试，任一成功即返回可访问
func (s *FriendService) dualCheck(ctx context.Context, targetURL string) (status int, latency int) {
	// 路径1：第三方 API 测速
	accessible1, latency1 := s.apiCheck(ctx, targetURL)
	if accessible1 && latency1 > 0 {
		return 0, latency1
	}

	// 路径2：直连检测
	accessible2, elapsed2, body := s.checkAccessibility(ctx, targetURL)
	if body != nil {
		defer func() { _ = body.Close() }()
	}
	latency2 := int(elapsed2.Milliseconds())
	if accessible2 {
		return 0, latency2
	}

	// 双方都失败，返回异常（优先使用 API 的延迟）
	if latency1 > 0 {
		return 1, latency1
	}
	return 1, latency2
}

// apiCheck 通过第三方 API 检测网站可访问性和延迟
func (s *FriendService) apiCheck(ctx context.Context, targetURL string) (bool, int) {
	apiURL := "https://v2.xxapi.cn/api/speed?url=" + url.QueryEscape(targetURL)
	client := &http.Client{Timeout: 15 * time.Second}
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, apiURL, nil)
	req.Header.Set("Authorization", "Bearer d1d4b155a12c03c0")

	resp, err := client.Do(req)
	if err != nil {
		return false, 0
	}
	defer func() { _ = resp.Body.Close() }()

	body, _ := io.ReadAll(resp.Body)
	var result struct {
		Code int    `json:"code"`
		Msg  string `json:"msg"`
		Data string `json:"data"` // 格式: "103ms"
	}
	if json.Unmarshal(body, &result) != nil {
		return false, 0
	}

	if result.Code != 200 {
		return false, 0
	}

	// 解析延迟值，提取数字部分（去掉 "ms" 后缀）
	latencyStr := strings.TrimSuffix(result.Data, "ms")
	latency, err := strconv.Atoi(latencyStr)
	if err != nil {
		return true, 0
	}

	return true, latency
}

// ============ 友链检测 ============

// CheckAllFriends 检测所有友链（定时任务调用）
// API 限制 50次/秒，每次批量10个，间隔200ms
func (s *FriendService) CheckAllFriends() error {
	ctx := context.Background()

	friends, err := s.repo.GetAllForCheck(ctx)
	if err != nil {
		return err
	}

	batchSize := 10
	delayMs := 200 // 每批次间隔200ms，控制不超过50次/秒

	for i := 0; i < len(friends); i += batchSize {
		end := i + batchSize
		if end > len(friends) {
			end = len(friends)
		}

		for j := i; j < end; j++ {
			friend := &friends[j]
			newStatus, latency := s.dualCheck(ctx, friend.URL)
			if newStatus > 0 {
				newStatus = friend.Accessible + 1
				if s.notificationService != nil {
					_ = s.notificationService.NotifyFriendAbnormal(ctx, friend.ID, friend.Name, newStatus)
				}
			} else {
				// 尝试自动发现 RSS（仅对无 RSS 配置的友链）
				s.discoverAndSaveRSS(ctx, friend)
			}
			_ = s.repo.UpdateCheckStatus(ctx, friend.ID, newStatus, latency)
		}

		// 批次之间添加延迟，避免超过API限速
		if end < len(friends) {
			time.Sleep(time.Duration(delayMs) * time.Millisecond)
		}
	}

	return nil
}

// checkAccessibility 检测网站可访问性，返回(是否可访问, 响应时间, 响应体)
func (s *FriendService) checkAccessibility(ctx context.Context, targetURL string) (bool, time.Duration, io.ReadCloser) {
	start := time.Now()
	client := &http.Client{Timeout: 10 * time.Second}

	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, targetURL, nil)
	req.Header.Set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")

	resp, err := client.Do(req)
	elapsed := time.Since(start)
	if err != nil {
		return false, elapsed, nil
	}

	accessible := resp.StatusCode >= 200 && resp.StatusCode < 300
	return accessible, elapsed, resp.Body
}

// discoverRSSFeed 从 HTML 中自动发现 RSS/Atom 订阅地址
func (s *FriendService) discoverRSSFeed(body io.Reader, baseURL string) string {
	if body == nil {
		return ""
	}

	doc, err := goquery.NewDocumentFromReader(body)
	if err != nil {
		return ""
	}

	var feedURL string
	doc.Find("link[type]").Each(func(_ int, sel *goquery.Selection) {
		typ, _ := sel.Attr("type")
		href, _ := sel.Attr("href")
		if href == "" || feedURL != "" {
			return
		}
		// 匹配 RSS 或 Atom MIME 类型
		if strings.Contains(typ, "application/rss+xml") ||
			strings.Contains(typ, "application/atom+xml") ||
			strings.Contains(typ, "application/feed+json") ||
			strings.Contains(typ, "application/xml") {
			// 处理相对路径
			if strings.HasPrefix(href, "/") || !strings.Contains(href, "://") {
				u, err := url.Parse(baseURL)
				if err == nil {
					ref, err := url.Parse(href)
					if err == nil {
						feedURL = u.ResolveReference(ref).String()
					}
				}
			} else {
				feedURL = href
			}
		}
	})

	// 排除评论 feed（如 Waline、Artalk 等）
	if feedURL != "" && (strings.Contains(feedURL, "comment") || strings.Contains(feedURL, "waline") || strings.Contains(feedURL, "artalk")) {
		feedURL = ""
	}

	return feedURL
}

// discoverAndSaveRSS 检测并自动保存友链的 RSS 地址
func (s *FriendService) discoverAndSaveRSS(ctx context.Context, friend *model.Friend) {
	// 已有 RSS 配置则跳过
	if friend.RSSUrl != "" {
		return
	}

	accessible, _, body := s.checkAccessibility(ctx, friend.URL)
	if body != nil {
		defer func() { _ = body.Close() }()
	}
	if !accessible || body == nil {
		return
	}

	feedURL := s.discoverRSSFeed(body, friend.URL)
	if feedURL != "" {
		_ = s.repo.UpdateRSSUrl(ctx, friend.ID, feedURL)
		logger.Info("自动发现友链 RSS: %s -> %s", friend.URL, feedURL)
	}
}
