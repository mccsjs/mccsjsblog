...
func NewArticleService(articleRepo *repository.ArticleRepository, tagRepo *repository.TagRepository, categoryRepo *repository.CategoryRepository, commentRepo *repository.CommentRepository, fileService *FileService, db *gorm.DB, cacheClient *cache.Client) *ArticleService {
...
func (s *ArticleService) GetBySlug(ctx context.Context, slug string) (*dto.ArticleDetailResponse, error) {
	...
	if s.cacheClient != nil {
		cacheKey := cache.BuildKey("article", slug)
		var cached dto.ArticleDetailResponse
	...
	if s.cacheClient != nil {
		cacheKey := cache.BuildKey("article", slug)
		_ = s.cacheClient.Set(ctx, cacheKey, response)
	}
