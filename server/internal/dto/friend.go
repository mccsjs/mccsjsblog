...
type FriendInGroupResponse struct {
	...
	Accessible  int    `json:"accessible"`  // 0=正常, -1=忽略检查, >0=连续失败次数
	Latency     int    `json:"latency"`     // 最后检测延迟(毫秒), 0=未检测
	IsFeatured  bool   `json:"is_featured"` // 是否推荐显示在页脚
}
...
type FriendListResponse struct {
	...
	Latency     int             `json:"latency"` // 最后检测延迟(毫秒)
	IsFeatured  bool            `json:"is_featured"` // 是否推荐
}

// FriendCheckResponse 友链测速响应
type FriendCheckResponse struct {
	Accessible int `json:"accessible"` // 可访问性: 0=正常, >0=异常
