...
func (v *FileValidator) Validate(file *multipart.FileHeader, cfg *config.UploadConfig, skipSizeValidation bool) error {
	if file == nil {
		return errors.New("文件不能为空: " + fmt.Sprintf("%v", file))
	}

	if strings.TrimSpace(file.Filename) == "" {
		return errors.New("文件名不能为空: " + file.Filename)
	}

	// 只有在不跳过大小验证时才检查文件大小
	if !skipSizeValidation && cfg.MaxFileSize > 0 && file.Size > cfg.MaxFileSize*1024*1024 {
		return fmt.Errorf("文件大小超出限制，文件大小: %d, 最大允许: %dMB", file.Size, cfg.MaxFileSize)
	}
	...
	if !allowed {
		return fmt.Errorf("不支持的文件类型: %s, 文件名: %s", contentType, file.Filename)
	}
...
func (m *Manager) DeleteFileByStorageType(filePath string, storageType string) error {
	...
	switch storageType {
	case "local":
		targetStorage = storage.NewLocalStorage("/srv/server/uploads")
	case "s3":
...
func ExtractHostFromContext(c *gin.Context) string {
	// 1. 检查是否有反向代理的 header
	scheme := "http"
