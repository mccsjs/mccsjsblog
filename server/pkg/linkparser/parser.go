...
type Metadata struct {
	...
	Avatar      string `json:"avatar"` // 站长头像（优先 og:image，其次 apple-touch-icon，最后 favicon）
}
...
func Parse(urlStr string) (*Metadata, error) {
	...
	// 确保 Avatar 是完整 URL
	if metadata.Avatar != "" && !strings.HasPrefix(metadata.Avatar, "http") {
		metadata.Avatar = resolveURL(parsedURL, metadata.Avatar)
	}

	// Avatar 优先用 og:image / apple-touch-icon，没找到则回退到 favicon
	if metadata.Avatar == "" {
		metadata.Avatar = metadata.Favicon
	}
...
func parseMeta(n *html.Node, metadata *Metadata) {
	...
	case "og:image":
		if metadata.Avatar == "" && content != "" {
			metadata.Avatar = content
		}
	}
...
func parseLink(n *html.Node, metadata *Metadata) {
	...
	// apple-touch-icon 可作为头像
	if strings.Contains(rel, "apple-touch-icon") && metadata.Avatar == "" {
